    ...
    while ($usuario = $resultado->fetch_assoc()) {
        ...
        foreach ($tipos_recordatorios as $tipo => $datos) {
            ...
            if ($es_momento_recordatorio) {
                // CAMBIO IMPORTANTE: En lugar de verificar si ya se enviu00f3 cualquier recordatorio hoy,
                // ahora verificamos si se enviu00f3 un recordatorio en la u00faltima hora para este mismo tipo
                // Esto permite enviar mu00faltiples recordatorios del mismo tipo en diferentes momentos del du00eda
                ...
                if ($resultado_verificacion->num_rows == 0 && !$ya_ficho) {
                    ...
                    if (enviarCorreo($correo, $asunto, $mensaje, "$nombre $apellidos")) {
                        // CAMBIO IMPORTANTE: Registrar la hora exacta del envu00edo
                        // Esto permite hacer una verificaciu00f3n por tiempo en lugar de por fecha
